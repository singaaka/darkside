...
import (
	...
	"github.com/singaaka/darkside-node-manager/internal/config"
...
func RegisterJobHandlers(s *Server) {
	...
	q.Register("migrate_paas", func(ctx context.Context, job dbgen.Job, logf func(string)) error {
...
func handleMigratePaas(ctx context.Context, s *Server, job dbgen.Job, logf func(string), runner *ansible.Runner) error {
	...
	if oldNode != nil {
		logf("==> Migrating darkside-paas data from " + oldNode.PublicIP + " to " + newNode.PublicIP)
		if err := runner.Run(ctx, ansible.RunOptions{
			Playbook:  "migrate_paas.yml",
			Inventory: "-",
			ExtraVars: map[string]string{
