...
func (s *Server) handleGitHubWebhook(w http.ResponseWriter, r *http.Request) {
	// 1. Load GitHub App config (for the webhook secret + private key).
	...
	if event != "push" {
		// Quietly accept anything else — we only act on push for v1.
		w.WriteHeader(http.StatusOK)
		return
	}
	...
	if branch == p.Ref {
		// Not a branch push (tag, etc.) — skip.
		w.WriteHeader(http.StatusOK)
		return
	}
...
// buildJob implements buildqueue.Job by wiring the Builder up to the store +
// hub bundled on the Server.
...
func (j *buildJob) Run(ctx context.Context) {
	b := &builder.Builder{
		...
		WorkRoot: j.s.opts.Config.DataDir + "/builds",
	}
