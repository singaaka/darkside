...
import { listUsers, whitelistUser, getOperatorConfig, type OperatorConfig } from "@/lib/api"

export const Route = createFileRoute("/settings")({
  component: SettingsPage,
})

type Tab = "github" | "operator" | "users"

function SettingsPage() {
  const { user } = useAuthStore()
  const [tab, setTab] = useState<Tab>("github")
  const tabs: Tab[] = ["github", "operator"]
        ...
        <nav className="-mb-px flex gap-6">
          {tabs.map((t) => (
            <button key={t} type="button" onClick={() => setTab(t)}
              className={`py-3 text-sm border-b-2 transition-colors capitalize ${tab === t ? "border-foreground text-foreground" : "border-transparent text-muted-foreground hover:text-foreground"}`}>
              {t === "github" ? "GitHub App" : t === "operator" ? "Operator config" : "Users"}
            </button>
          ))}
        </nav>
      </div>
...
function OperatorTab() {
  const cfg = useRQQuery({ queryKey: ["operator-config"], queryFn: getOperatorConfig })
  ...
  const rows: Array<[string, keyof OperatorConfig, string?]> = [
    ["Version", "version"],
    ["External URL", "external_url", "Public HTTPS URL GitHub callbacks land at."],
    ["Host", "host"],
    ["Listen", "listen", "host:port the binary binds to inside its container."],
    ["Data dir", "data_dir"],
    ["Nomad", "nomad_addr"],
    ["Registry", "registry_addr", "Private docker registry the build job pushes to."],
    ["Docker host", "docker_host"],
  ]

  return (
    <section className="space-y-3 rounded-lg border p-5">
      <h2 className="text-lg font-medium">Operator config</h2>
      <p className="text-sm text-muted-foreground">
        What <code className="font-mono">darkside.toml</code> resolved to at startup. Read-only —
        edit the file and restart the container to change.
      </p>
      <dl className="grid grid-cols-[max-content_1fr] gap-x-4 gap-y-2 text-sm pt-2">
        {rows.map(([label, key, hint]) => (
          <div key={key} className="contents">
            <dt className="text-muted-foreground">{label}</dt>
            <dd className="font-mono break-all">
              {String(cfg.data[key])}
              {hint && <div className="text-xs text-muted-foreground font-sans mt-0.5">{hint}</div>}
            </dd>
          </div>
        ))}
      </dl>
